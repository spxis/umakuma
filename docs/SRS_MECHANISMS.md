...
Legend: **have** · **partial** · **don't** · **won't**

| # | Mechanism | Who does it | Us | Why, or why not |
|---|---|---|---|---|
| A | **Leech detection**: surface an item that keeps failing, and suspend or flag it | Anki: 8 lapses → tag + suspend, warn every 4 after. SuperMemo: lapses *and* current interval, with a one-lapse-early "semi-leech" warning. Skritter built it and switched it off. WaniKani: none. | **don't** | Nothing detects a leech. The simulator says low accuracy is a tax not a wall (a 60% learner is 2.2× slower, never stuck) so it is not urgent — but WaniKani is the *only* surveyed system without one, and that is not company to keep. SuperMemo's conjunctive rule is the one to copy: an item that survives a long gap stops being a leech regardless of history. |
...
| E | **A side channel** for the failing item so it cannot block the main line | Bunpro ghost reviews: a miss spawns a separate 4 h / 12 h / 24 h / 48 h track while the main schedule advances. WaniKani Extra Study / Recent Mistakes (24 h, no SRS effect). Anki filtered decks. | **don't** | The best structural fit we do not have. Bunpro's "Minimal" setting (only after two misses on the same item) is the right default. |
| F | **Throttle intake off the backlog** automatically | Anki, by default: the review limit also caps new cards, so introduction pauses when you are behind. WaniKani leaves it to the learner and their own docs show it does not work. | **partial** | The level gate is a throttle in effect: you cannot start material from levels you have not unlocked, so `lessonsPerDay` is a ceiling a struggling learner never reaches. Within a level, nothing throttles. The simulator implemented and measured the Anki rule (`throttleLessonsOnBacklog`): average backlog **−85%** for 0.8% of progress. **Modelled, not shipped.** Highest value-for-nothing change the simulator found. |
