...
- **A responsive hide is written as a variant, never as a bare `hidden`.** Two plain Tailwind utilities in the same property group are resolved by stylesheet order, not by the order they appear in the `class` attribute - so `hidden` does not beat the `inline-flex` a shared base class already applied, and `hidden sm:inline-flex` on anything carrying `FILTER_CHIP_CLASS` renders visible at every width. Write `max-sm:hidden`. The study explorer's level, grouping and status rows were all written the first way and none of them ever collapsed on a phone; the only symptom was the `...` that opens them being drawn beside chips that never hid, because `sm:hidden` on that button *is* a variant and did win. There is no error, no warning and nothing a unit test over the source could see - `LevelFilterChips.test.tsx` sweeps for the old spelling now. Check a responsive rule by screenshotting at 393px, not by reading the classes.
- **A filter row costs one line on a phone, and a long list of levels folds into decades.** `CompactFilterRow` hides every chip but the one that is on until the row is opened, and `LevelFilterChips` (over `ladderLevelChips`) turns a hundred levels into ten group chips, any one of which opens into its levels when pressed. History drew one chip per level - sixty-three of them, thirteen lines - above the list somebody opened the page for. Both are shared: a new filter row uses them rather than mapping over its values.
...
- **`public/assets/currency/` is built, not edited.** The three mascots and their reaction faces are masters in `src/images/currency/` (one 128-unit SVG each, named `<currency>` or `<currency>-<reaction>`), and `pnpm currency:build` writes every file the folder holds: base, `-dark`, optical sizes `-18` to `-64` in both, and transparent PNGs `-128` to `-2048`. It also deletes anything there no master produces. To change a face, change the master and rebuild; to drop a detail below a size, mark it `data-min-size="32"` in the master. The list of what exists is `GAME_CURRENCY_REACTIONS` in `src/lib/gameCurrencyReactions.ts`, and its test fails if the masters, the built files and the list disagree in any direction. The nine reactions arrived as one sticker sheet whose standalone exports were broken by shared clip ids; the build namespaces every id by asset name so a sheet can never do that again.
...
- **The timeline is the shipped record; the board that says what is open is
  ...
  `pnpm backlog add`, `claim`, `release` and `file` are **retired** and exit
  non-zero saying so: planned work is not written into `featureTimeline.json`
  any more. `file` was the last to go and was the one that bit, because it
  succeeded: it wrote a `planned` entry into the JSON and marked the ticket
  `filed`, which looks like progress and is not. A versionless entry is
  invisible rather than merely wrong - `release` counts only entries that
  carry a version, so nothing on `/admin/releases` would ever show it - and
  the ticket had left the board. If you find one from before it was retired,
...
- **Shared state has more than one writer, and the second one imports the rules rather than restating them.** The ticket board is written by `pnpm task`, by `release:take` and by the admin page. The CLI writes `claimedBy`, `claimedAt` and `status` together and puts the condition inside the `updateMany`, so the database refuses a takeover; the admin page's PATCH route validated only that the destination was *a* status, and so it could reopen a shipped ticket and clear the `filedAs` pointing at its release, leave a ticket `in_progress` that nobody held, or mark one shipped with no entry behind it — the one state the rest of the system says cannot exist. Every refusal came back as `404 No such wish` and the row swallowed it, so a refused press looked like nothing happening. Fixed in 1.113.0 and 1.114.0 by making the legal moves a rule the route asks (`canMoveTicket`), the write the CLI's own shape (`ticketMoveData` + `ticketMoveWhere`), and `shipped` a type error to offer. **The rows were clean the whole time**, because nobody had pressed the buttons: an absence of bad data is evidence nobody has used a writer, not evidence the writer is correct. When you change the rules in `src/lib/tickets.ts`, open `scripts/tasks.ts` and `scripts/release-take.ts` in the same pass.
...
- **`pnpm db:backup:prod` before every write to the production database.** Schema
  ...
  **The database is in the command's name: `db:backup:prod` and
  `db:backup:dev`.** There was one `db:backup` and it took
  `DIRECT_URL ?? DATABASE_URL`; a worktree's `.env` points `DIRECT_URL` at the
  local container, so an inline `DATABASE_URL` for Neon was never reached and
  it dumped `127.0.0.1` while printing "Backup verified: 43 table(s) with
  data". That is how you delete from production holding a backup of nothing.
  `db:backup:prod` needs nothing exported from anywhere - it finds production
  by asking git for the main working tree and reading the `.env` beside it -
  and the bare `db:backup` now refuses and names the two. The line it prints,
  `Backing up <hostname> to ...`, comes from the URL handed to `pg_dump` and
  cannot disagree with what was dumped; read it anyway. On 2026-09-05 production was changed four times in one session - two
