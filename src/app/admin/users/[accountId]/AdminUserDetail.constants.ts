...
export const ADMIN_USER_DETAIL_COPY = {
  ...
  activity: {
    heading: "How they are getting on",
    blurb:
      "Derived from their XP history, which is the best day-by-day record the site has. Rest days and vacation days hold the streak but are not study, so they never count as activity.",
    xp: "XP",
    rank: "Rank",
    ...
    averagePerDay: "Average per active day",
    bestDay: "Best day",
    restDays: "Rest days",
    ...
    streakLine: (current: number, longest: number) => `${current} now, ${longest} at their best`,
    streakToday: "counted today",
    streakAtRisk: "not yet today",
    usedOfAllowed: (used: number, allowed: number) => `${used} of ${allowed} used`,
    grantedNote: (granted: number) => `includes ${granted} granted`,
    bestDayLine: (dayKey: string, amount: number) => `${amount} XP on ${dayKey}`,
    awayHeading: "Away right now",
    awayLine: (back: string) => `Back on ${back}`,
    awaySince: (since: string) => `Away since ${since}`,
    awayOverdue: "Their return date has passed and the vacation is still running.",
    homeLine: "Not on vacation.",
  },

  timeOff: {
    heading: "Time off",
    blurb:
      "The allowance comes from their rank, over a rolling year. A grant is added on top of it rather than replacing it, so climbing a rank still helps them and a later change to the rules still reaches them.",
    kind: "Kind",
    kindRest: "Rest days",
    kindVacation: "Vacation days",
    days: "Days",
    daysHint: "Added to this year's allowance. Whole days.",
    note: "Reason (optional)",
    noteHint: "For the admin reading this back in a month. The member never sees it.",
    grant: "Grant days",
    granting: "Granting...",
    granted: (days: number, kind: string) => `Granted ${days} ${kind.toLowerCase()}.`,
    grantFailed: "Could not grant those days.",
    confirmGrantTitle: "Grant time off",
    confirmGrantDescription: (nickname: string, days: number, kind: string) =>
      `Scope: 1 account (${nickname}). Effect: ${days} extra ${kind.toLowerCase()} for the coming year, on top of what their rank earns. Risk: additive, and undone only by deleting the row. Continue?`,
    confirmGrantLabel: "Grant it",
    endVacation: "End their vacation",
    ending: "Ending...",
    ended: (shifted: number, items: number) =>
      `Vacation ended. Their queue moved ${shifted} day(s) forward across ${items} item(s).`,
    endedNotAway: "They were not away.",
    endFailed: "Could not end that vacation.",
    confirmEndTitle: "End vacation early",
    confirmEndDescription: (nickname: string) =>
      `Scope: 1 account (${nickname}). Effect: every review due date shifts forward by however long they were away, and unused days go back to their allowance. Risk: not reversible - the shift cannot be unwound. Continue?`,
    confirmEndLabel: "End it",
    grantsHeading: "Grants on file",
    grantsEmpty: "Nothing granted yet.",
    grantLine: (days: number, kind: string) => `${days} ${kind}`,
    grantExpired: "no longer counting",
  },
