              ...
              <tr className="transition hover:bg-surface-muted/80">
                ...
                {activeTab === LEADERBOARD_TABS.overall ? (
                  <>
                    ...
                    <td className="px-4 py-3 text-xs uppercase tracking-[0.08em] text-foreground/60"><p>{row.lastActivityAt ? formatDate(row.lastActivityAt) : "-"}</p><p className="mt-1 text-[10px] font-semibold normal-case tracking-normal text-foreground/60">{formatSince(row.lastActivityAt)}</p></td>
                  </>
                ) : (
                  <>
