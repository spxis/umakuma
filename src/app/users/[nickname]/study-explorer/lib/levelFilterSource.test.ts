...
describe("the level filter asks the item's own ladder", () => {
  ...
  /* John: "All level filters don't show up and don't work in UK mode. In WK
     they worked appropriately." A UmaKuma item carries ukLevel and no wkLevel,
...
describe("queueItemLevel", () => {
  it("takes ours where we have one, and WaniKani's otherwise", () => {
    expect(queueItemLevel({ ukLevel: 11, wkLevel: 3 })).toBe(11);
    expect(queueItemLevel({ ukLevel: null, wkLevel: 3 })).toBe(3);
    expect(queueItemLevel({ ukLevel: 11, wkLevel: undefined })).toBe(11);
  });

  it("is null for an item on no ladder, so it is skipped rather than counted at zero", () => {
    expect(queueItemLevel({ ukLevel: null, wkLevel: undefined })).toBeNull();
    expect(queueItemLevel({ ukLevel: 0, wkLevel: 0 })).toBeNull();
  });
});
