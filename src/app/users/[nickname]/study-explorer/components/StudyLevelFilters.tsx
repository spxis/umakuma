  ...
  return (
    <div className="flex w-full max-w-full items-start gap-1 rounded-xl border border-line bg-surface px-1.5 py-1" role="tablist" aria-label="Level filters">
      ...
      <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1">
        ...
        >
          All <span className="ml-1 inline-block align-text-bottom text-[10px] font-semibold leading-tight text-foreground/55">({formatNumber(queueMode === STUDY_QUEUE_TYPES.lesson ? totalLessonsInVisibleLevels : totalReviewsInVisibleLevels)})</span>
        </button>
        {queueMode === STUDY_QUEUE_TYPES.lesson
          ? lessonLevelOptions.map(([level, count]) => (
              ...
              >
                {level} <span className="ml-1 inline-block align-text-bottom text-[10px] font-semibold leading-tight text-foreground/55">({formatNumber(count)})</span>
              </button>
            ))
          : reviewLevelChips.map((chip) => {
              if (chip.kind === "range") {
                ...
                return (
                  ...
                  >
                    ...
                    {isGroupedChip ? (
                      <span className="ml-1 inline-block align-text-bottom text-[10px] font-semibold leading-tight text-foreground/55">({formatNumber(count)})</span>
                    ) : null}
                  </button>
                );
              }
              ...
              return (
                ...
                >
                  {chip.level} <span className="ml-1 inline-block align-text-bottom text-[10px] font-semibold leading-tight text-foreground/55">({formatNumber(levelCount)})</span>
                </button>
              );
            })}
        {isCollapsedOnMobile ? (
