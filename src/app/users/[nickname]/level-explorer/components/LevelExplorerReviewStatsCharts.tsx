...
export function SuccessFailureSplitChart({ success, failure }: { success: number; failure: number }) {
  const total = Math.max(0, success) + Math.max(0, failure);
  const safeTotal = Math.max(1, total);
  const successPct = Math.round((Math.max(0, success) / safeTotal) * 100);
  const failurePct = Math.round((Math.max(0, failure) / safeTotal) * 100);
  const difficulty =
    total < 8 ? "Insufficient data" : successPct >= 90 ? "Easy" : successPct >= 75 ? "Medium" : "Hard";
    ...
    <div>
      ...
      <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
        <div className="rounded border border-emerald-200 bg-emerald-50 px-2 py-1">
          <p className="font-bold uppercase text-emerald-700">Success</p>
          <p className="mt-1 font-black text-emerald-800">{success}</p>
        </div>
        <div className="rounded border border-rose-200 bg-rose-50 px-2 py-1">
          <p className="font-bold uppercase text-rose-700">Failure</p>
          <p className="mt-1 font-black text-rose-800">{failure}</p>
        </div>
        <div className="rounded border border-line bg-surface px-2 py-1">
          ...
          <p className="mt-1 font-black text-foreground">{successPct}%</p>
          <p className="text-[10px] text-foreground/60">{total} reviews</p>
        </div>
...
export function CorrectWrongTrendChart({ points }: { points: TrendPoint[] }) {
  if (points.length < 2) {
    return (
      <div>
        <p className="mb-1 text-sm font-bold">Success/Failure Review Trend</p>
        <p className="text-xs text-foreground/60">Need at least two reviews to draw this trend.</p>
      </div>
    );
  }
  ...
  const maxCount = Math.max(1, ...points.map((point) => Math.max(point.success, point.failure)));
  ...
  const successPolyline = points.map((point) => `${toX(point.timeMs)},${toY(point.success)}`).join(" ");
  const failurePolyline = points.map((point) => `${toX(point.timeMs)},${toY(point.failure)}`).join(" ");
  ...
  return (
    <div>
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="h-[200px] w-full rounded border border-line bg-surface">
        ...
        <polyline fill="none" strokeWidth="2" className="text-emerald-600" stroke="currentColor" points={successPolyline} />
        <polyline fill="none" strokeWidth="2" className="text-rose-600" stroke="currentColor" points={failurePolyline} />
      </svg>
...
export function SuccessRateTrendChart({ points }: { points: SuccessRatePoint[] }) {
  if (points.length < 2) {
    return (
      <div>
        <p className="mb-1 text-sm font-bold">Success Rate Review Trend</p>
        <p className="text-xs text-foreground/60">Need at least two reviews to draw this trend.</p>
      </div>
    );
  }
...
export function ReviewActivityTrendChart({ points }: { points: ActivityPoint[] }) {
  if (points.length < 2) {
    return (
      <div>
        <p className="mb-1 text-sm font-bold">Review Activity</p>
