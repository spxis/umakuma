...
export const metadata: Metadata = {
  ...
  description: "News history is available from your user dashboard.",
};
...
export default async function NewsHistoryPage() {
  ...
  if (linkedAccount?.wkUsername) {
    redirect(`/users/${encodeURIComponent(linkedAccount.wkUsername)}/news?read=history`);
  }
  ...
  if (invitePayload?.accountId) {
    ...
    if (inviteAccount?.wkUsername && inviteAccount.inviteCodeHash) {
      redirect(`/users/${encodeURIComponent(inviteAccount.wkUsername)}/news?read=history`);
    }
  }

  return (
    <main className="mx-auto w-full max-w-2xl px-4 py-16 text-center">
      <h1 className="text-xl font-black text-foreground sm:text-2xl">News history moved to user dashboards</h1>
      <p className="mt-3 text-sm text-foreground/75 sm:text-base">
        Open your user page to see read history.
      </p>
      <div className="mt-6">
        <Link
          href="/join"
