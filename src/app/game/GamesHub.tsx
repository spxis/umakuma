          ...
          return (
            <li key={card.kind}>
              <button
                ...
                className={`flex h-full w-full flex-col rounded-2xl border-2 bg-surface p-4 text-left transition hover:bg-surface-muted focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-accent/40 disabled:cursor-not-allowed disabled:opacity-45 disabled:hover:bg-surface sm:p-5 ${isSelected ? accent.border : "border-line"}`}
              >
                <span className="flex items-start justify-between gap-3">
                  <span className="flex min-w-0 items-center gap-2">
                    <span aria-hidden="true" className="text-2xl leading-none">{GAME_KIND_EMOJI[card.kind]}</span>
                  ...
                  {card.statusLabel ? (
                    <span className="shrink-0 whitespace-nowrap rounded-full border border-line bg-surface-muted px-2.5 py-1 text-[10px] font-black uppercase text-foreground/65">
                      {card.statusLabel}
                    </span>
                  ) : null}
                </span>
                ...
                <span className={`mt-4 inline-flex h-10 items-center justify-center rounded-lg border px-5 text-xs font-black uppercase ${card.playable ? accent.solid : "border-line bg-surface-muted text-foreground/50"}`}>
                  {card.playable ? GAME_COPY.play : GAME_COPY.notEnoughItems}
                </span>
              </button>
            </li>
          );
