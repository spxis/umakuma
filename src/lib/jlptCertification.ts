...
export type JlptCertificationInput = {
  status: JlptCertificationStatus;
  year?: number | null;
  level?: number | null;
  system?: JlptSystem | null;
};

export type JlptCertificationValidation =
  | { ok: true; system: JlptSystem | null; level: number | null; year: number | null }
  | { ok: false; error: string };

/**
 * Validates one answer from the registration form.
 *
 * `currentYear` is passed in rather than read from the clock so the caller
 * controls the boundary; a planned sitting may be in the future, a passed one
 * may not.
 */
export function validateJlptCertification(
  input: JlptCertificationInput,
  currentYear: number,
): JlptCertificationValidation {
  const { status } = input;

  if (status === JLPT_CERTIFICATION_STATUSES.none || status === JLPT_CERTIFICATION_STATUSES.undisclosed) {
    return { ok: true, system: null, level: null, year: null };
  }

  const year = input.year ?? null;
  const level = input.level ?? null;

  if (status === JLPT_CERTIFICATION_STATUSES.passed) {
    if (year === null) {
      return { ok: false, error: "Tell us the year you passed." };
    }
    if (!Number.isInteger(year) || year < JLPT_FIRST_YEAR || year > currentYear) {
      return { ok: false, error: `The year must be between ${JLPT_FIRST_YEAR} and ${currentYear}.` };
    }

    const system = input.system ?? jlptSystemForYear(year);
    if (system === null) {
      return { ok: false, error: "That year is before the JLPT existed." };
    }
    if (level === null) {
      return { ok: false, error: "Tell us which level you passed." };
    }
    ...
    return { ok: true, system, level, year };
  }

  // Planned: a future sitting always uses the modern system, and the year is
  // optional because plenty of people know the level before the date.
  if (level !== null && !isModernLevel(level)) {
    return { ok: false, error: "Pick a level from N5 to N1." };
  }
  if (year !== null && (!Number.isInteger(year) || year < currentYear)) {
    return { ok: false, error: "A planned sitting must be this year or later." };
  }

  return { ok: true, system: JLPT_SYSTEMS.modern, level, year };
}
