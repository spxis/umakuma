import type { SimPersona } from "./simTypes";

/**
 * Twenty-four people, with lives.
 *
 * The first set were adjectives — devoted, steady, gamer — and they were fine
 * for pacing arithmetic and useless for judging whether the system is fair.
 * "Ambitious" does not tell you why somebody vanishes for three weeks; "a
 * teacher in her first year" does, and it tells you it will happen again every
 * September.
 *
 * Each carries a `story` because the numbers underneath it are a claim about a
 * real person, and a claim you can read is a claim you can argue with. If a
 ...
 * is one unbroken absence, which is where the vacation rules get tested.
 * Accuracy is the cruellest number in here: it decides how many times an item
 * has to be answered before it burns, and at 65% that is 27 answers where at
 * 92% it is 9.
 *
 * The last four arrive from WaniKani part way up the ladder, which is the case
 * the import question is about — see `simImport.ts`.
 */

type PersonaInput = Omit<SimPersona, "startLevel" | "startXp"> & Partial<Pick<SimPersona, "startLevel" | "startXp">>;

/** Everybody starts at the bottom with nothing unless they say otherwise. */
function person(input: PersonaInput): SimPersona {
  return { startLevel: 1, startXp: 0, ...input };
}
...
export const SIM_PERSONAS: SimPersona[] = [
  ...
  person({
    id: "morning-and-night",
    label: "Twice a day, morning and night",
    story:
      "The control for the session-timing question, and the reason it can be answered at all: identical to the evening-only learner below in every number except when they sit down.",
    attendance: 0.9, reviewsPerDay: 45, lessonsPerDay: 10, gamesPerDay: 2, accuracy: 0.85, sessionHours: [8, 21], sitsExams: true, holidayDays: 14,
  }),
  person({
    id: "once-a-day",
    label: "Once a day, evenings only",
    story:
      "The same person as above with one sitting instead of two. Whatever separates the two rows is what a second sitting is worth, and nothing else.",
    attendance: 0.9, reviewsPerDay: 45, lessonsPerDay: 10, gamesPerDay: 2, accuracy: 0.85, sessionHours: [21], sitsExams: true, holidayDays: 14,
  }),
  person({
    id: "wanikani-import-17",
    label: "Arrived from WaniKani 17, imported to UmaKuma 20",
    story:
      "Two years of somebody else's reviews behind him and an XP rank of one. He is the exact case John asked about: standing at level 20 on the ladder and at the bottom of the other one, next to a member who earned the same level here.",
    attendance: 0.8, reviewsPerDay: 60, lessonsPerDay: 12, gamesPerDay: 2, accuracy: 0.85, sessionHours: [8, 21], sitsExams: true, holidayDays: 14,
    startLevel: 20,
  }),
  person({
    id: "wanikani-import-30",
    label: "Arrived from WaniKani 30, imported to UmaKuma 40",
    story:
      "Further up and impatient with anything below where she stands. The larger the import, the larger the gap between the two ladders on day one.",
  ...
  person({
    id: "wanikani-import-60",
    label: "Arrived from WaniKani 60, imported to UmaKuma 85",
    story:
      "Finished WaniKani outright. The most extreme import there is, and the one that decides whether an import award can be a flat number.",
    attendance: 0.7, reviewsPerDay: 70, lessonsPerDay: 12, gamesPerDay: 2, accuracy: 0.9, sessionHours: [8, 21], sitsExams: true, holidayDays: 21,
    startLevel: 85,
  }),
];

export function simPersonaById(id: string): SimPersona | null {
  return SIM_PERSONAS.find((persona) => persona.id === id) ?? null;
}
