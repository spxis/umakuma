...
/**
 * Every release's name, in version order.
 *
 * Separated from the rules in `releaseCodenames.ts` for one reason: this list
 * grows by a line on every single release and nothing else in that file grows
 * at all. Keeping them together meant the 500-line gate would eventually fail
 * on a release that had done nothing wrong — which is exactly what happened at
 * 0.429.0, and the fix is a split rather than a bigger limit.
 *
 * The rules — the gojūon walk, the kana each minor must start on, the ban on
